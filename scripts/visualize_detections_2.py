...
def generate_risk_image():
    ...
    print("Loading model and checkpoint...")
    config = load_config('/tmp/object_detection/configs/default.yaml')
    # Use float() instead of half() to prevent Infinity values in YOLO output
    model = WRDNet(config).cuda().eval().float() 
    ckpt = torch.load('/checkpoints/phase1/best.pth', map_location='cuda')
    model.load_state_dict(ckpt['model_state_dict'])
    print("Model loaded successfully.")
    ...
    for img_path in image_paths:
        ...
        img = cv2.imread(img_path)
        img_resized = cv2.resize(img, (1024, 512))
        # Use float() for input tensor as well
        img_tensor = torch.from_numpy(img_resized).permute(2, 0, 1).unsqueeze(0).float().cuda() / 255.0
